...
7. Business-data disconnect means:
   A. Data is encrypted
   B. Data insights are misaligned with business goals
   C. Too many dashboards
   D. Only IT can access data

8. Diagnostic analytics answers:
   A. What happened?
   B. Why did it happen?
   C. What will happen?
   D. What should we do?

9. The decision-action gap means:
   A. Too much data
   B. Decisions are made without data
   C. Insights exist but are not acted on
   D. Data is deleted too early

10. Predictive analytics answers:
   A. What happened?
   B. Why did it happen?
   C. What will happen?
   D. What should we do?

11. Organizational silos mainly refer to:
   A. Too many dashboards
   B. Lack of communication between departments
   C. Too many backups
   D. Slow internet

12. Prescriptive analytics answers:
   A. What happened?
   B. Why did it happen?
   C. What will happen?
   D. What should we do?

13. Netflix collects viewing data mainly to:
   A. Reduce storage costs only
   B. Understand engagement and guide content decisions
   C. Replace all BI tools
   D. Avoid dashboards

14. A KPI is best described as:
   A. A raw data table
   B. A database schema
   C. A measurable metric tied to a business goal
   D. A SQL join
...
24. What does `import math` do?
   A. Deletes a module
   B. Imports the math module for use
   C. Creates a new file
   D. Runs a script

25. What is `sum([1, 2, 3])`?
   A. 3
   B. 5
   C. 6
   D. 7

26. What is `{"a": 1, "b": 2}["b"]`?
   A. 1
   B. 2
   C. "b"
   D. Error

27. What is `[x * 2 for x in [1, 2, 3]]`?
   A. [2, 4, 6]
   B. [1, 2, 3]
   C. [2, 3, 4]
   D. Error

28. What is `"a,b,c".split(",")`?
   A. ["a,b,c"]
   B. ["a", "b", "c", ""]
   C. ["a", "b", "c"]
   D. Error

29. For `items = ["a", "b", "c"]`, what is `items[-1]`?
   A. "a"
   B. "b"
   C. "c"
   D. Error

30. Which statement about dictionaries is true?
   A. Keys are always ordered alphabetically
   B. Keys can be mutable lists
   C. Duplicate keys create multiple entries
   D. Keys must be unique
...
42. SQL JOIN is used to:
   A. Back up data
   B. Combine rows from multiple tables
   C. Delete a table
   D. Change column types

43. Many-to-many relationships usually use:
   A. A linking table
   B. A single column only
   C. A file folder
   D. A view only

44. Which ACID property ensures transactions don't interfere?
   A. Atomicity
   B. Consistency
   C. Isolation
   D. Durability

45. Which database type is best for flexible JSON-like documents?
   A. Document database
   B. Graph database
   C. Time-series database
   D. Relational database
...
72. Unsupervised learning uses:
   A. Inputs with labels y
   B. Only rewards
   C. Inputs without labels
   D. Only rules

73. The learning rate in gradient descent is:
   A. A type of dataset
   B. The step size for updates
   C. A measure of accuracy
   D. A storage limit

74. Training typically repeats until:
   A. Loss stops improving
   B. The dataset is deleted
   C. Labels are removed
   D. Queries fail

75. Which is a supervised learning example?
   A. Grouping customers by similarity
   B. Finding clusters without labels
   C. Classifying emails as spam using labeled data
...
11-B, 12-D, 13-B, 14-C, 15-A, 16-A, 17-C, 18-C, 19-B, 20-B,
21-A, 22-B, 23-A, 24-B, 25-C, 26-B, 27-A, 28-C, 29-C, 30-D,
31-A, 32-A, 33-D, 34-A, 35-B, 36-D, 37-C, 38-A, 39-B, 40-B,
41-B, 42-B, 43-A, 44-C, 45-A, 46-B, 47-A, 48-C, 49-A, 50-B,
51-B, 52-B, 53-C, 54-A, 55-D, 56-A, 57-B, 58-C, 59-D, 60-B,
61-A, 62-B, 63-D, 64-B, 65-B, 66-A, 67-B, 68-C, 69-A, 70-B,
71-A, 72-C, 73-B, 74-A, 75-C, 76-A, 77-C, 78-B, 79-B, 80-D,
81-A, 82-A, 83-A, 84-A, 85-A, 86-A, 87-B, 88-C, 89-D, 90-A
